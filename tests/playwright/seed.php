...
$moduleid = $DB->get_field('modules', 'name', ['name' => 'flexaccess'], IGNORE_MISSING)
    ? $DB->get_field('modules', 'id', ['name' => 'flexaccess'], MUST_EXIST)
    : null;
$cmid = 0;
if ($moduleid) {
    $moduleinfo = add_moduleinfo((object) [
        'course' => $course->id,
        'module' => $moduleid,
        'modulename' => 'flexaccess',
        'section' => 0,
        'visible' => 1,
        'name' => 'Secure my account for later',
        'intro' => 'Keep your progress by turning this temporary access into a permanent account.',
        'introformat' => FORMAT_HTML,
    ], $course);
    $cmid = (int) ($moduleinfo->coursemodule ?? 0);
}
echo "export FLEXACCESS_CMID='" . $cmid . "'\n";
